...
- **학생 명단** — 학번·이름·성별 관리 (수동 / 엑셀·CSV / 클립보드 붙여넣기, 성별 자동 인식, 엑셀 양식 내려받기)
- **좌석 설정** — 좌석 *공간* 정의 — 분단·행·열, 빈자리(비움), 남녀 자리 구역
- **제약** — 자리 고정·회피, 짝 거리(가깝게/멀리), 앞자리 등. PIN 잠금, 우선순위 드래그
- **배정** — 동성짝·이성짝(둘 다 켜면 무작위)·회피 옵션 → 배정/확정, 수동 교체, 자동 완화 배너
- **기록** — 확정 배치 누적 — 다음 배정의 같은 자리·같은 짝 회피 기준
- **꾸미기** — 스킨 9종·글꼴 선택·배경/셀 이미지·남녀 색 구분·교탁 기준 → 자리표 **PNG 출력**(미리보기 = 저장본)
- **정보** — 데이터 저장 위치·안내
